...
namespace SRE {
	...
	void int_write(void *dest, int n, unsigned int value)
	{
		...
		case 2:
			...
			break;
		case 3:   //注意：严格来讲，这里需要考虑大端 小端的问题（这里暂不考虑，按小端来处理）
	...
	{
		//对于DDS格式
		if (isCompressed(format))  //暂时只支持DDS的纹理压缩格式，OpenGL的暂不支持
		{
			switch (format)
			{
			case PF_DXT1:  //DX1压缩比为4:1
				return ((width + 3) / 4) * ((height + 3) / 4) * 8 * depth;
			case PF_DXT2:  //DX2和DX3都是带了64位的alpha信息，压缩比为2:1
			case PF_DXT3:
				return ((width + 3) / 4) * ((height + 3) / 4) * 16 * depth;
			case PF_DXT4: //DX4和DX5所带的64为alpha信息，也采用了索引结构
			case PF_DXT5:
				return ((width + 3) / 4) * ((height + 3) / 4) * 16 * depth;
			}
		}
