...
vec3 BRDF_Specular_BlinnPhong( const in IncidentLight incidentLight, const in GeometricContext geometry, const in vec3 specularColor, const in float shininess ) {
	...
	vec3 F = F_Schlick( specularColor, dotLH );//为菲涅尔反射函数( Fresnel 函数 )

	float G = G_BlinnPhong_Implicit( /* dotNL, dotNV */ );//G为阴影遮罩函数（Geometry Factor，几何因子），即未被shadow或mask的比例

	float D = D_BlinnPhong( shininess, dotNH );//D为法线分布函数(NDF)
